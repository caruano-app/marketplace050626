    ...
    <div className="min-h-screen bg-neutral-100">
      ...
      <main className="mx-auto max-w-[1412px] px-4 py-5 pb-24">
        <section className="rounded-[8px] bg-neutral-950 p-5 text-white">
          ...
          <h1 className="mt-1 text-3xl font-black uppercase leading-tight">Movimentacao economica</h1>
          <p className="mt-2 max-w-3xl text-sm font-bold text-neutral-300">
            Vendas locais, ordens de abastecimento e produtos mais bipados no Agreste.
          </p>
        </section>
        ...
        <section className="mt-4 flex gap-3 overflow-x-auto pb-2">
          <article className="min-w-[190px] rounded-[8px] bg-[#ffd700] p-4 text-neutral-950 shadow-sm">
            <p className="text-xs font-black uppercase">Vendas PDV</p>
          ...
          <article className="min-w-[220px] rounded-[8px] bg-[#00a86b] p-4 text-white shadow-sm">
            <p className="text-xs font-black uppercase">Total vendido local</p>
          ...
          <article className="min-w-[190px] rounded-[8px] bg-white p-4 text-neutral-950 shadow-sm">
            <p className="text-xs font-black uppercase text-neutral-500">Ordens B2B</p>
            <p className="mt-2 text-3xl font-black">{data.orders.length}</p>
          </article>
        </section>
              ...
              <article className="rounded-[8px] border border-neutral-200 p-4" key={order.id}>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm font-bold text-neutral-600">Fornecedor: {order.supplierName}</p>
                  </div>
        ...
        <section className="mt-4 grid gap-4 lg:grid-cols-[1fr_0.9fr]">
                ...
                <article className="rounded-[8px] border border-neutral-200 p-4" key={sale.id}>
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs font-bold text-neutral-500">{sale.storeName} | SKU {sale.sku || "-"}</p>
                    </div>
                    <p className="text-lg font-black text-[#f58220]">{formatPrice(sale.valor_total)}</p>
                  </div>
                  <p className="mt-2 text-sm font-bold text-neutral-700">
                    Quantidade: {sale.quantidade} | {formatDate(sale.criado_em)}
                  </p>
                </article>
              ))}
              {!data.sales.length ? (
          ...
          <div className="rounded-[8px] bg-white p-4 shadow-sm">
            <h2 className="text-xl font-black uppercase text-neutral-950">Produtos mais bipados</h2>
            <div className="mt-4 grid gap-3">
